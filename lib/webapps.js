...
Webapps.prototype = extend(ClientMethods, {
  ...
  installPackagedWithADB: function(path, appId, cb) {
    ...
    function createTemporaryFolder() {
      var c = child_process.spawn("adb", ["shell", "mkdir -p /data/local/tmp/b2g/" + appId], {stdio:"inherit"});
      c.on("close", uploadPackage);
    }
    // then upload the package to the temporary directory
    function uploadPackage() {
      var child = child_process.spawn("adb", ["push", path, "/data/local/tmp/b2g/" + appId + "/application.zip"], {stdio:"inherit"});
